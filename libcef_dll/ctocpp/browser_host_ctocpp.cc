...
//
// $hash=d7e5ee4fe2656a8abe9a2b5b2ae50ee11b395870$
//
...
NO_SANITIZE("cfi-icall")
void CefBrowserHostCToCpp::ReleaseAcceleratedPaintSurface(uint64_t surface_id) {
  shutdown_checker::AssertNotShutdown();

  auto* _struct = GetStruct();
  if (!_struct->release_accelerated_paint_surface) {
    return;
  }
  ...
  // Execute
  _struct->release_accelerated_paint_surface(_struct, surface_id);
}
#endif  // CEF_API_ADDED(CEF_EXPERIMENTAL)

#if CEF_API_ADDED(CEF_EXPERIMENTAL)
NO_SANITIZE("cfi-icall")
void CefBrowserHostCToCpp::SetAcceleratedPaintSpareSurfaces(uint32_t count) {
  shutdown_checker::AssertNotShutdown();

  auto* _struct = GetStruct();
  if (!_struct->set_accelerated_paint_spare_surfaces) {
    return;
  }
  ...
  // Execute
  _struct->set_accelerated_paint_spare_surfaces(_struct, count);
}
#endif  // CEF_API_ADDED(CEF_EXPERIMENTAL)
